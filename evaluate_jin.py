...
def evaluate(
    ...
    if start_from_scratch:
        nn = ActorCritic2(env.map_size, recurrent)
    else:
        nn = load_model2(nn_path, env.map_size, recurrent)
    ...
    agents = [Agent2(model=nn, smooth_sample_ratio=0) for _ in range(env.players_num)]
    print(f"-------------------------\n players_num is {env.players_num}")
    ...
    for _ in range(env.max_episodes):
        ...
        while not obses_t[0].done:
            ...
            for i in range(len(players)):
                ...
                # _st = time.time()
                if stochastic:
                    action = agents[i].think(obses=obses_t[i], debug=args.debug, way="stochastic", accelerator=device,
                                             mode="eval")
                else:
                    action = agents[i].think(obses=obses_t[i], debug=args.debug, way="deterministic",
                                             accelerator=device, mode="eval")
                if not fast_forward:
                    print(action)
                    input()
...
#   python3 evaluate.py --env-id fullgame-v0 -stc --episodes 10  --model-path rl599.pth --opponent WorkerRush
#   python3 evaluate.py --env-id LargeMapTest-v0 -stc --episodes 10 --model-path rl199.pth --opponent LightRush
#   python3 evaluate.py --env-id EvalAgainstPassive-v0 -stc --episodes 2  --model-path rl699.pth --opponent LightRush -ff

#   Namespace(debug=False, env_id='fullgame-v0', episodes=2, fast_forward=True, model_path='rl699.pth', opponent='LightRush', recurrent=False, render=1, stc=True)