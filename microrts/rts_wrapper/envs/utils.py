...
# dataclass asdict -> {"name1":value,"name2":value}
# json.dumps python object -> json string
# json.loads json -> python list (dict): [{'name':value,' ':value2}]
...
#         pa = PlayerAction(unitID=unit.ID)  # it is better to say the unit action assignment
#
...
#
#         # if there exist a legal action,  or a legal possible action then create an instance;
#         # else replace with none action
...
#                 return i
#
#     def action_type_idx(u_type):
#         nna_idx[idx_action_type] = u_type
#
#     def action_parameter_idx(ua_parameter):
#         nna_idx[idx_action_parameter] = ua_parameter
#
#     def action_unit_type_idx(ua_unit_type):
#         if ua_unit_type == "":
#             nna_idx[idx_produce_type] = -1
#         elif ua_unit_type == UNIT_TYPE_NAME_BASE:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[0]
#         elif ua_unit_type == UNIT_TYPE_NAME_BARRACKS:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[1]
#         elif ua_unit_type == UNIT_TYPE_NAME_WORKER:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[2]
#         elif ua_unit_type == UNIT_TYPE_NAME_LIGHT:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[3]
#         elif ua_unit_type == UNIT_TYPE_NAME_HEAVY:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[4]
#         elif ua_unit_type == UNIT_TYPE_NAME_RANGED:
#             nna_idx[idx_produce_type] = AGENT_COLLECTION[5]
#
#     if ua.type == ACTION_TYPE_ATTACK_LOCATION:
#         nna_idx[1] = attack_trans(u.x, u.y, ua.x, ua.y)
#     else:
#         nna_idx[1] = action_parameter_idx(ua.parameter)
#
#     nna_idx[0] = action_type_idx(ua.type)
#
#     nna_idx[2] = action_unit_type_idx(ua.unitType)
#     return nna_idx
...
#
#     # 5 + 6 + 7
#     channel_action_type = np.zeros((5, h, w))
#     channel_action_parm = np.zeros((6, h, w))
#     channel_action_prod_type = np.zeros((7, h, w))
#
#     for action in gs.actions:
#         _id = action.ID  # the executor id of the action
#         _action = action.action
...
#         channel_action_parm[_one_hot_action_prod_type][_x][_y] = 1
#
#     spatial_features = np.vstack(
#         (
#             channel_whether_walkable,  # 2
...
#             # 43
#
#             channel_action_type,  # 6
#             channel_action_parm,  # 5
#             channel_action_prod_type  # 7
#
#             # 50 + 11
#
#         ),
...
# get data from csv, and then convert to numpy or tensor directly except unit type
def utt_encoder(utt_str: str):
    ...
    utt_feature = np.hstack(
        (
            encoder_dict[UNIT_TYPE_NAME_BASE],
            encoder_dict[UNIT_TYPE_NAME_BARRACKS],
            encoder_dict[UNIT_TYPE_NAME_WORKER],
            encoder_dict[UNIT_TYPE_NAME_LIGHT],
            encoder_dict[UNIT_TYPE_NAME_RANGED],
            encoder_dict[UNIT_TYPE_NAME_HEAVY],
            encoder_dict[UNIT_TYPE_NAME_RESOURCE],
        )
    )
    return utt_feature, encoder_dict, ans.size
...
utt_feature, encoded_utt_dict, encoded_utt_feature_size = utt_encoder(UTT_ORI)
