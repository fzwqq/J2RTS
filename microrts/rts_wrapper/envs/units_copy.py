...
def action_sampler_v2(model, state, info, device='cpu', mode='stochastic', hidden_states: dict = None, callback=None,
    ...
    for uva in unit_valid_actions:
        ...
        if _type == UNIT_TYPE_NAME_BASE and info["resources"] <= 0:
            continue
        if _type == UNIT_TYPE_NAME_BARRACKS and info[
            "resources"] <= 1:  # resource less than the lowest cost of production
            continue
...
#     print(unit_inv  stance_encoder(8, 8, unit))
