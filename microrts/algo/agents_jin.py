from microrts.rts_wrapper.envs.utils import network_simulator
...
class Agent2:
    ...
    def __init__(self, model, memory_size=10000, random_rollout_steps=128, smooth_sample_ratio=None, map_size=(4, 4)):
        ...
        self.memory = ReplayBuffer2(memory_size)
    ...
    def ctf_adv(self, info, device):
        ...
        unit, _, _, _ = ua
        u_f = torch.from_numpy(unit_feature_encoder_v2(unit, self.map_size)).unsqueeze(0).float().to(device)
    ...
    def think(self, sp_ac=None, callback=None, way="stochastic", debug=False, **kwargs):
        ...
        # [(uva, ua_type, ua_parm, ua_prod)]
        network_unit_actions = [(s[0].unit, s[1], s[2], s[3]) for s in samples]
        ...
        if mode == "train" and sampler is not network_simulator:
            ...
            for u, ua_type, ua_parm, ua_prod in network_unit_actions:
                _id = str(u.ID)
